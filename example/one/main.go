...
func main() {
	...
	// Start web server
	//http.Handle(pattern("/html/"), http.StripPrefix(pattern("/html/"), http.HandlerFunc(dwnld.Fileserver)))
	//http.HandleFunc("/", printpath)
	err := http.ListenAndServe(":8789", http.HandlerFunc(dwnld.Fileserver))
	if err != nil {
		fmt.Println("Error")
	}
}
